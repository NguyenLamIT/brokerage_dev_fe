                    ...
                    <div className='flex justify-between' key={index}>
                        <Link href={
                            "/" + comment.role == "BUYER"
                                ...
                                : "supplier" + "/" + comment.user.name + "*" + comment.user.code
                        } className='flex gap-4'>
                            <Image src={comment.user.avatar} alt='Logo' width={44} height={44} className='h-11 w-11 rounded-full object-cover' />
                            <div className='flex flex-col gap-1'>
                                <div className='text-sm font-bold text-[#081342] flex gap-1 items-center'>{comment.user.name} <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-blue-600">
                                    <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clipRule="evenodd" />
                                </svg>
                                    <p className='text-xs text-[#8C8585]'>{formatDistanceToNow(new Date((comment.created_at)), { addSuffix: true, locale: enAU })}</p>
                                </div>
