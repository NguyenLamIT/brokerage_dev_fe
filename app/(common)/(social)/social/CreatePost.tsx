...
const CreatePost = ({ user }: any) => {
  ...
  const createPost = () => {
    if (!ref?.current?.value) {
      toast({
        variant: "destructive",
        title: "Fail",
        description: "Please fill in all required fields content",
      });
      return
    }
    setLoading(true);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-2 w-full bg-white p-4 rounded-lg border border-gray-200">
        {user && (
          <Image
            unoptimized
            src={user?.role == "SELLER"? user?.company.logo_seller: user?.company.logo_buyer}
            alt={user.last_name}
            width={45}
            height={45}
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <div>
                <div className="flex gap-4 justify-end py-4 cursor-pointer">
                  <p>Add multiple videos or photos into your post</p>
                  <div
                  ...
                  ></input>
                  {/* <div className="flex gap-3 items-center text-[#515151]"  onClick={() => uploadFileRef?.current?.click()}>
                    <Image
                      src={"/videocam.png"}
                      alt="videocam"
                      width={24}
