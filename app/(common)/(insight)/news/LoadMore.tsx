...
const LoadMore = ({ length, total }: any) => {
    ...
    const fetchData = () => {
        setLoading(true);
        getRequest("/insight/news?total_page=12" + "&page=" + page + "&category_code=" + search + "&keyword=" + keyword)
            .then((data) => {
        ...
        <div className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-[1.6875rem] gap-y-[3.125rem]">
                {data.map((dt: any, index: any) => (
                    <NewsItem key={index} value={dt} />
                ))}
            </div>
            {length + data.length < total && (
                <div className="w-full flex py-[3.125rem] justify-center items-center">
                    {loading ? (
                        <Button disabled>
